...
// 添加店铺
const handleAdd = () => {
  getSellsUpload({ path: window.location.href }).then(
...
// 获取店铺列表
const getStoreList = () => {
  AuthStoreGoodsList({ page: currentPage.value, limit: pageSize.value }).then((res: any) => {
    if (res.status == 200) {
...
<style scoped src="./UpStore.scss"></style>
